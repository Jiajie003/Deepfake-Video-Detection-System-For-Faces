...
app.secret_key = 'yc&7Jasf823!@Xlmz2#9Zb0pwU'  # 修改为自己的密钥
...
def upload_file():
    ...
    if request.method == 'POST':
        ...
        face_images, crop_faces, original_video_path, preview_video_path, results = analyze_video(video_path, unique_folder_name)
...
def analyze_video(filepath, unique_folder_name, num_frames=20):
    ...
    for i in range(total_frames):
        ...
        if i in frame_indices:
            ...
            if len(faces) > 0:
                ...
                crop_face_images.append(f"{unique_folder_name}/frames_crop/{crop_filename}")
    ...
    for i in range(num_frames):
        ...
        face_images.append(f"{unique_folder_name}/frames/{filename}")
        results.append({
    ...
    relative_video_path = f"{unique_folder_name}/result_preview.mp4"
    return face_images, crop_face_images, filepath.replace("\\", "/"), relative_video_path, results
